...
<CardGroup cols={2}>
  ...
  <Card title="Powered by Browserbase" icon="browser">
    Run browser automation anywhere with just an API key - no local browser
    required
  </Card>
...
<Step title="Set up credentials">
  ```bash mcpkit secrets ``` You'll need: - Browserbase API key ([get one
  here](https://www.browserbase.com)) - LLM API key (Gemini, OpenAI, etc.)
</Step>
